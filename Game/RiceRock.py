...
def draw(canvas):
    ...
    my_ship.draw(canvas)
    my_ship.update()
    process_spirit_group(rock_group,canvas)
    process_spirit_group(explosion_group,canvas)
    process_spirit_group(missile_group,canvas)
...
def process_spirit_group(group,canvas):
    remove = set([])
    for sprite in group:
        if sprite.update():
            remove.add(sprite)
...
def group_group_collide (groupOne, groupTwo):
    ...
    for sprite in groupOne:
        if group_collide (groupTwo, sprite) > 0:
            ...
            Collisions += 1
    if len (remove) > 0:
        groupOne.difference_update (remove)
    return Collisions    
# initialize stuff
