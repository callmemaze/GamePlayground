...
class Ship:
    ...
    def in_angle_vel(self):
        self.angle_vel += 0.03
    
    def de_angle_vel(self):
        self.angle_vel -= 0.03
        
    def update_thrust(self,thrustOn):
        self.thrust = thrustOn
        if self.thrust:
...
def draw(canvas):
    ...
    canvas.draw_text ('Lives: '+ str(lives), (50, 50), 36, "Aqua")
    canvas.draw_text ('Score: '+ str(score), (625, 50), 36, "Aqua")
...
def keydown(key):
    if key == simplegui.KEY_MAP["left"]:
        my_ship.de_angle_vel()
    elif key == simplegui.KEY_MAP["right"]:
        my_ship.in_angle_vel()
